...
from flask import jsonify, send_file, session, request, render_template, redirect, Blueprint
...
from models import UTMLink, ExcludedOption, Campaign, ClicksDate, GoogleAnalyticsDataGraph, GoogleAnalyticsDataTable, \
    Blogger
from utils.db_utils import get_campaign_info, generate_campaign_excel, get_graph_info
from utils.ga4_requests import build_analytics_request_table, build_analytics_request_graph
from utils.short_link import update_clicks_count, create_short_link, edit_link, \
    delete_link
...
def index():
    ...
    unique_url, unique_campaign_content, unique_campaign_source, unique_campaign_medium, unique_campaign_name = unique_list_tags()
    ...
    if short_secure_url is None:
        return render_template('index.html', unique_campaign_contents=unique_campaign_content,
...
def campaigns():
    ...
    for campaign in campaigns:
        ...
        campaign_data["data"] = sorted(campaign_data["data"], key=lambda x: (x["session_source_medium"], x["date"]))

        # Добавление структуры кампании в общий список
        all_sessions_and_dates.append(campaign_data)
    return render_template('campaigns.html', aggregate_campaign_data=aggregate_campaigns,
...
def filter_setting():
    if request.method == 'POST':
        ...
        url = request.form.getlist('url')  # `getlist` для получения всех выбранных значений
        ...
        links = query.all()
        for link in links:
            clicks = ClicksDate.query.with_entities(func.sum(ClicksDate.clicks_count)) \
                         .filter(ClicksDate.link_id == link.id,
            ...
            sessions, users = GoogleAnalyticsDataGraph.query \
                                  .with_entities(
                func.sum(GoogleAnalyticsDataGraph.sessions),
                func.sum(GoogleAnalyticsDataGraph.active_users)) \
                                  .filter(GoogleAnalyticsDataGraph.date >= start_date,
...
def analytics_data():
    ...
    for campaign in campaigns:
        for property_id in Config.PROPERTY_IDS:
            ...
            request = build_analytics_request_graph(property_id, path, start_date, end_date)
            ...
            # Создаем запрос с фильтром
            request = build_analytics_request_table(property_id, path, start_date, end_date)
            ...
            for row in response.rows:
                ...
                # Check if the record already exists
                existing_record = GoogleAnalyticsDataTable.query.filter_by(session_source_medium=session_source_medium,
                                                                           url=url, content=content).first()
                if existing_record:
                    # Update existing record
...
def bloggers_page():
    ...
    if request.method == "POST":
        ...
        if yt_name:
            ...
            update_bloggers_avg()
            return render_template('new_observ.html', error_message="Added", bloggers=bloggers)
        else:
            return render_template('new_observ.html', error_message="NOT found", bloggers=bloggers)
