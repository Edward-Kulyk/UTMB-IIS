...
def index():
    ...
    unique_url, unique_campaign_content, unique_campaign_source, unique_campaign_medium, unique_campaign_name = (
        unique_list_tags()
    )
...
def manage_exclusions():
    ...
    excluded_campaign_sources = (
        ExcludedOption.query.filter(ExcludedOption.option_type == "campaign_sources[]").distinct().all()
    )
    excluded_campaign_mediums = (
        ExcludedOption.query.filter(ExcludedOption.option_type == "campaign_mediums[]").distinct().all()
    )
    excluded_campaign_contents = (
        ExcludedOption.query.filter(ExcludedOption.option_type == "campaign_contents[]").distinct().all()
    )
    excluded_campaign_names = (
        ExcludedOption.query.filter(ExcludedOption.option_type == "campaign_names[]").distinct().all()
    )
...
def campaigns():
    ...
    campaigns = Campaign.query.filter(not_(Campaign.hide)).all()
    ...
    if request.method == "POST":
        ...
        for link in links:
            clicks = (
                ...
                .filter(ClicksDate.link_id == link.id, ClicksDate.date >= start_date, ClicksDate.date <= end_date)
                .scalar()
                or 0
            )
            ...
            sessions, users = GoogleAnalyticsDataGraph.query.with_entities(
                func.sum(GoogleAnalyticsDataGraph.sessions), func.sum(GoogleAnalyticsDataGraph.active_users)
            ).filter(
...
def get_default_values():
    ...
    print(campaign_name)
    return jsonify(
        {"url_by_default": default_values.url_by_default, "domain_by_default": default_values.domain_by_default}
    )
...
def query():
    ...
    list_names = []
    if request.method == "GET":
        campaign_names = Campaign.query.filter(not_(Campaign.hide)).all()
        ...
        for property_id in Config.PROPERTY_IDS:
            ...
            for data in data_to_insert:
                ...
                existing_record = GoogleAnalyticsDataGraph.query.filter_by(
                    session_source_medium=data["session_source_medium"], url=data["url"], date=data["date"].date()
                ).first()
                if existing_record:
            ...
            for row in response.rows:
                ...
                existing_record = GoogleAnalyticsDataTable.query.filter_by(
                    session_source_medium=session_source_medium, url=url, content=content
                ).first()
                if existing_record:
                    # Update existing record
