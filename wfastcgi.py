...
class FastCgiRecord(object):
    ...
    def __repr__(self):
        return "<FastCgiRecord(%d, %d, %d, %d)>" % (self.type, self.req_id, self.role, self.flags)
...
def read_fastcgi_get_values(stream, req_id, content):
    ...
    send_response(stream, req_id, FCGI_GET_VALUES_RESULT, write_fastcgi_keyvalue_pairs(response))
...
def get_environment(dir):
    web_config = os.path.join(dir, "src/web.config")
...
try:
    from _winapi import CreateFile, CloseHandle, GetLastError, ExitProcess, WaitForSingleObject, INFINITE, OPEN_EXISTING
except ImportError:
