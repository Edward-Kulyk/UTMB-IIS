...
def update_clicks_count():
    ...
    utm_links = (
        UTMLink.query.join(Campaign, UTMLink.campaign_name == Campaign.name)
        .filter(Campaign.hide == False)
        .all()
    )
    ...
    for utm_link in utm_links:
        ...
        response_data = get_clicks_filter(
            utm_link.short_id, utm_link.campaign.start_date, current_date
        )
        ...
        for dataset in datasets:
            ...
            for click in data:
                click_date = datetime.strptime(
                    click["x"], "%Y-%m-%dT%H:%M:%S.%fZ"
                ).date()
                new_clicks_count = int(click["y"])

                # Проверяем, существует ли запись для данной даты и ссылки
                link_click_date = ClicksDate.query.filter_by(
                    link_id=utm_link_id, date=click_date
                ).first()
                ...
                if link_click_date:
                    ...
                    link_click_date = ClicksDate(
                        link_id=utm_link_id,
                        date=click_date,
                        clicks_count=new_clicks_count,
                    )
...
def edit_link(short_id):
    record = UTMLink.query.filter_by(id=short_id).first()
...
def delete_link(short_id):
    url = f"https://api.short.io/links/{short_id}"
