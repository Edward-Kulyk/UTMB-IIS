...
from app import db
from models import UTMLink, ClicksDate, GoogleAnalyticsDataGraph, GoogleAnalyticsDataTable
...
def get_top_5_source_medium_from_ga4(url):
    ...
    top_source_medium = db.session.query(GoogleAnalyticsDataGraph.session_source_medium,
                                         func.sum(GoogleAnalyticsDataGraph.sessions).label('total_sessions')).filter_by(
        url=url) \
...
def get_sessions_and_dates_from_ga_graph(url, source_mediums):
    ...
    query_results = GoogleAnalyticsDataGraph.query.filter_by(url=url) \
...
def get_campaign_info(campaign):
    ...
    campaign_clicks_total = ClicksDate.query.with_entities(func.sum(ClicksDate.clicks_count)).filter(
        ClicksDate.link.has(UTMLink.campaign == campaign)
    ).scalar() or 0

    campaign_clicks_1d = ClicksDate.query.with_entities(func.sum(ClicksDate.clicks_count)).filter(
        ClicksDate.link.has(UTMLink.campaign == campaign),
        ClicksDate.date == campaign.start_date
    ).scalar() or 0

    campaign_clicks_7d = ClicksDate.query.with_entities(func.sum(ClicksDate.clicks_count)).filter(
        ClicksDate.link.has(UTMLink.campaign == campaign),
        and_(
            ClicksDate.date > campaign.start_date,
            ClicksDate.date <= campaign.start_date + timedelta(days=7)
        )
    ).scalar() or 0

    campaign_clicks_14d = ClicksDate.query.with_entities(func.sum(ClicksDate.clicks_count)).filter(
        ClicksDate.link.has(UTMLink.campaign == campaign),
        and_(
    ...
    campaign_clicks_21d = ClicksDate.query.with_entities(func.sum(ClicksDate.clicks_count)).filter(
        ClicksDate.link.has(UTMLink.campaign == campaign),
        and_(
    ...
    for link in links:
        ...
        source_medium = f"{link.campaign_source} / {link.campaign_medium}"
        ga_info = GoogleAnalyticsDataTable.query.filter_by(url=campaign.url_by_default,
                                                           session_source_medium=source_medium,
                                                           content=link.campaign_content).first()
        if ga_info:
            ...
            if ga_info.average_session_duration < 60:
                ...
                session_duration = f"{round(ga_info.average_session_duration) // 60}m {round(ga_info.average_session_duration) % 60}s"
        ...
        for click in clicks:
            ...
            if click.link.campaign.start_date + timedelta(
                    days=1) <= click.date <= click.link.campaign.start_date + timedelta(days=7):
                link_info['clicks_7d'] += click.clicks_count
            if click.link.campaign.start_date + timedelta(
                    days=7) < click.date <= click.link.campaign.start_date + timedelta(days=14):
                link_info['clicks_14d'] += click.clicks_count
            if click.link.campaign.start_date + timedelta(
                    days=14) < click.date <= click.link.campaign.start_date + timedelta(days=21):
...
def get_graph_info(campaign):
    ...
    sessions_and_dates_ga_graph = get_sessions_and_dates_from_ga_graph(campaign.url_by_default,
                                                                       top_5_source_medium_combined)
    # Получим сессии и даты из таблицы ClicksDate для текущей кампании
    sessions_and_dates_clicks = get_sessions_and_dates_for_campaign(campaign.url_by_default,
                                                                    top_5_source_medium_combined)
...
def generate_campaign_excel(campaign_info,graph_info, output):
    ...
    headers = ['URL', 'Campaign Content', 'Campaign Source', 'Campaign Medium', 'Short ID', 'Short Secure URL',
               'Clicks 24h', 'Clicks W1', 'Clicks W2', 'Clicks W3', 'Total Clicks', 'GA Active Users', 'GA Sessions',
