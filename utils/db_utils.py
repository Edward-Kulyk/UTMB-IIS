...
from app import db
from models import (ClicksDate, GoogleAnalyticsDataGraph,
                    GoogleAnalyticsDataTable, UTMLink)
...
def get_top_5_source_medium_for_campaign(campaign_url):
    ...
    top_5_source_medium = sorted(clicks_data.items(), key=lambda x: x[1], reverse=True)[
        :5
    ]
...
def get_campaign_info(campaign):
    ...
    campaign_clicks_14d = (
        ...
        .filter(
            ...
            and_(
                ClicksDate.date
                > campaign.start_date + timedelta(days=7),  # Следующий день после 7-го
                ClicksDate.date
                <= campaign.start_date + timedelta(days=14),  # До конца 14-го дня
            ),
        )
        .scalar()
        or 0
    ...
    campaign_clicks_21d = (
        ...
        .filter(
            ...
            and_(
                ClicksDate.date
                > campaign.start_date
                + timedelta(days=14),  # Следующий день после 14-го
                ClicksDate.date
                <= campaign.start_date + timedelta(days=21),  # До конца 21-го дня
            ),
        )
        .scalar()
        or 0
...
def get_graph_info(campaign):
    ...
    all_source_medium = set(
        [source_medium[0] for source_medium in ga4]
        + [source_medium[0] for source_medium in clicks]
    )
        ...
        for date in date_range:
            ...
            for session_data in sessions_and_dates_ga_graph:
                if (
                    ...
                    and session_data["source_medium"] == source_medium
                ):
            ...
            for click_data in sessions_and_dates_clicks:
                if (
                    click_data["date"] == date
                    and click_data["session_source_medium"] == source_medium
                ):
